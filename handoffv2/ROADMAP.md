...
| `feat/auth` | 🔴 | Register+verify, login, guest, Google/Facebook OAuth, refresh rotation, `/auth/me`; protected routes; the prototype's auth screen reproduced (sign-in / create-account tabs, socials, guest). |
| `feat/landing-onboarding` | 🟡 | Public marketing **Landing** page (hero, feature cards, how-to-play, footer) shown pre-auth → `Play Now`/`Sign In` enter auth; **Forgot-password** flow (email → "check your email" → resend); post-signup **Onboarding** carousel (4 slides, Skip, dots) gated by `fdr.onboarded`. All match the prototype. |
...
| `feat/system-states` | 🟡 | **Offline** banner (browser `online`/`offline`), **404** route fallback (unknown screen → branded not-found w/ Home + Play), **error** boundary (`componentDidCatch` → recovery screen w/ Reload + Home, shows error detail), **maintenance** mode (full-screen takeover at highest z-index, driven by a server/health flag + the admin maintenance toggle; "Check again" re-checks). All match the prototype. |
