...
| Loading screen art | `uploads/loading/*` — 6 scenes (matchmaking, crimson, jade, obsidian, throne, victory), each landscape + `-portrait` `.webp` |

> **Loading screen is a component, not just art.** The prototype ships `Loading Screen.dc.html` (copied into `handoff/`), imported by the main app as `<dc-import name="Loading Screen" context=… duration=…>`. In production, build it as its own React component (`features/loading/LoadingScreen.tsx`) taking a `context` (which scene) + `duration` prop, shown during matchmaking, match start, and results. It has a painted placeholder scene layer plus a Meshy-art image slot per context — the art files above fill those slots.
