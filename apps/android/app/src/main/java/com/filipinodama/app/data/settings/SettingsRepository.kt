...
object SettingsRepository {
    ...
    private suspend fun <T> call(block: suspend () -> ApiEnvelope<T>): SettingsResult<T> {
        return try {
            ...
            if (apiError != null) SettingsResult.Failure(apiError.code, apiError.message)
            else SettingsResult.Failure("NETWORK_ERROR", "Couldn't reach the server. Check your connection and try again.")
        }
    }
}
