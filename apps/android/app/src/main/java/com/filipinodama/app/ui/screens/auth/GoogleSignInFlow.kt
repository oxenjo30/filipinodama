 ...
 *     AuthPage.tsx's `disabled={!googleOn}` exactly: gated ONLY on
 *     providers.google, same as web (busy/loading state is handled
 *     separately by the screen, same as every other button there).
...
/** Mirrors AuthPage.tsx: `const googleOn = providers.google` gates the button, nothing else. */
fun googleButtonEnabled(providers: ProvidersResponse): Boolean = providers.google
