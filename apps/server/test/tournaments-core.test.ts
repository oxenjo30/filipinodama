...
describe("SWISS — progressive round generation (money-critical)", () => {
  ...
  it("BUG 2 — round-gen lost to the concurrent-report race (round fully done, round+1 never created): completeTournament must lazily recover it, not 409 forever", async () => {
    // Deterministically reproduces the STATE that the real READ COMMITTED
    // race produces, instead of depending on racy timing: under the race,
    // two concurrent transactions reporting the round's two DIFFERENT last
    // matches can each read the round as "not fully done yet" (seeing their
    // own update but not the other, still-uncommitted, transaction's
    // update), so BOTH skip maybeGenerateNextSwissRound and BOTH commit —
    // round+1 never gets created even though the round IS fully done.
    // Constructed directly here (both round-1 matches marked done via a raw
    // update, bypassing reportResult's generation step entirely) so the test
    // is 100% reliable rather than hoping a real race interleaves a
    // particular way. Before the fix, completeTournament 409s NOT_FINISHED
    // forever with no way out — this is the "tournament stranded" bug.
    const { tournament } = await openSwiss(4, { prizePoolGold: 1000, prizeSplitGold: [700, 300] });
    await startTournament(prisma, tournament.id, await actingAdmin());
    const round1 = await prisma.tournamentMatch.findMany({ where: { tournamentId: tournament.id, round: 1 }, orderBy: { slot: "asc" } });
    expect(round1.length).toBe(2);

    // Mark both round-1 matches "done" directly (as if resolveRoundRobinSlot
    // ran for both) WITHOUT ever calling maybeGenerateNextSwissRound — this
    // is exactly the end state the real race produces.
    for (const m of round1) {
      await prisma.tournamentMatch.update({ where: { id: m.id }, data: { winnerEntryId: m.redEntryId, status: "done", resolvedAt: new Date() } });
    }
    const round2Before = await prisma.tournamentMatch.findMany({ where: { tournamentId: tournament.id, round: 2 } });
    expect(round2Before.length).toBe(0); // confirms the stranded state: round fully done, round+1 missing

    // completeTournament must detect "round 1 fully done, not the final
    // round, round 2 missing" and generate round 2 itself (lazy recovery),
    // 409ing ONLY because round 2 isn't reported yet — never getting stuck
    // with round 2 permanently absent.
    await expect(completeTournament(prisma, tournament.id, await actingAdmin(), "trigger recovery")).rejects.toMatchObject({ status: 409, code: "NOT_FINISHED" });

    const round2 = await prisma.tournamentMatch.findMany({ where: { tournamentId: tournament.id, round: 2 } });
    expect(round2.length).toBe(2); // recovered — the stranded round was generated
  ...
  it("BUG 2 regression — a genuine Promise.all race on the round's two DIFFERENT last matches still lets the tournament reach completion (defense in depth over the deterministic test above)", async () => {
    const { tournament } = await openSwiss(4, { prizePoolGold: 1000, prizeSplitGold: [700, 300] });
    await startTournament(prisma, tournament.id, await actingAdmin());
    const round1 = await prisma.tournamentMatch.findMany({ where: { tournamentId: tournament.id, round: 1 }, orderBy: { slot: "asc" } });

    await Promise.all([
      reportResult(prisma, tournament.id, round1[0]!.id, round1[0]!.redEntryId!),
      reportResult(prisma, tournament.id, round1[1]!.id, round1[1]!.redEntryId!),
    ]);

    // Whether or not this particular run actually lost the race, the
    // tournament must always be able to reach round 2 and complete — if
    // round 2 didn't get generated by either racer, completeTournament's
    // lazy recovery must produce it.
    let round2 = await prisma.tournamentMatch.findMany({ where: { tournamentId: tournament.id, round: 2 } });
    if (round2.length === 0) {
      await expect(completeTournament(prisma, tournament.id, await actingAdmin(), "trigger recovery")).rejects.toMatchObject({ status: 409, code: "NOT_FINISHED" });
      round2 = await prisma.tournamentMatch.findMany({ where: { tournamentId: tournament.id, round: 2 } });
...
describe("DOUBLE_ELIM — winners+losers bracket, grand final, bracket reset (money-critical)", () => {
  ...
  describe("startTournament — seeds W bracket + pre-creates L skeleton + GF slot", () => {
    ...
    // n=3 (non-power-of-two) is now REJECTED at start by the DE_NEEDS_POWER_OF_TWO
    // gate (see "DE_NEEDS_POWER_OF_TWO gate" describe block below) — a bye
    // producing no loser used to leave the L bracket stuck pending forever,
    // permanently locking entry-fee gold. This test moved there.
  });
  ...
  describe("DE_NEEDS_POWER_OF_TWO gate — non-power-of-two field rejected at start (BUG 1: prevents permanently stuck money)", () => {
    // BEFORE this gate existed, a non-power-of-two DE field (e.g. n=3 with
    // maxPlayers=4) seeded a bracket with a W-round-1 bye. A bye produces NO
    // loser, so the losers-bracket slot that loser WOULD have dropped into
    // never gets its second competitor and stays "pending" forever — the
    // L-final never resolves, the Grand Final never fills, and
    // completeTournament 409s NOT_FINISHED indefinitely. Money in (entry
    // fees), no possible payout, no possible completion. The gate below
    // rejects Start outright for any non-power-of-two n so this can never
    // happen; admins can wait for a full bracket or cancel+refund instead.
    it.each([
      { n: 3, maxPlayers: 4 },
      { n: 5, maxPlayers: 8 },
      { n: 6, maxPlayers: 8 },
      { n: 7, maxPlayers: 8 },
    ])("n=$n (maxPlayers=$maxPlayers, not a power of two): start throws DE_NEEDS_POWER_OF_TWO, tournament stays OPEN", async ({ n, maxPlayers }) => {
      const entryFeeGold = 50;
      const t = await seedTournament({ status: "OPEN", format: "DOUBLE_ELIM", maxPlayers, entryFeeGold, prizePoolGold: 0, prizeSplitGold: [] });
      const players = [];
      for (let i = 0; i < n; i++) {
        const u = await seedUser({ gold: 1000 });
        const entry = await joinTournament(prisma, t.id, u.id);
        players.push({ user: u, entry });
      }

      await expect(startTournament(prisma, t.id, await actingAdmin())).rejects.toMatchObject({ status: 400, code: "DE_NEEDS_POWER_OF_TWO" });

      // Tournament never left OPEN — no bracket was seeded, no matches created.
      const after = await prisma.tournament.findUniqueOrThrow({ where: { id: t.id } });
      expect(after.status).toBe("OPEN");
      const matchCount = await prisma.tournamentMatch.count({ where: { tournamentId: t.id } });
      expect(matchCount).toBe(0);

      // MONEY UNTOUCHED: every player was charged the entry fee on join (as
      // normal) and that charge is still intact — no debit was lost, and
      // every player can still leave for a full refund since the tournament
      // is still OPEN.
      for (const p of players) {
        expect(await goldOf(p.user.id)).toBe(1000 - entryFeeGold);
      }
      for (const p of players) {
        await leaveTournament(prisma, t.id, p.user.id);
        expect(await goldOf(p.user.id)).toBe(1000); // fully refunded
      }
    });

    it("regression: power-of-two fields (n=4, n=8) still start successfully — the gate only rejects non-powers-of-two", async () => {
      for (const n of [4, 8]) {
        const { tournament } = await startedDoubleElimOf(n);
        const after = await prisma.tournament.findUniqueOrThrow({ where: { id: tournament.id } });
        expect(after.status).toBe("RUNNING");
        const wRound1 = await prisma.tournamentMatch.count({ where: { tournamentId: tournament.id, bracket: "W", round: 1 } });
        expect(wRound1).toBe(n / 2);
      }
    });
  });
});
