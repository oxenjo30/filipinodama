...
- [x] `fd-kpi` — KPI tile with per-tile value color override (green/amber/ink/gold) + a bottom trend/delta row (sub left, colored delta right)
- [x] `fd-2col` — 1fr/1fr (and 1.35fr/1fr) two-column card-row grid, gap 14
- [x] `abtn` / `btnGold` — gold gradient pill/button (+ full-width variant)
- [x] `btnGhost` / `btnAmber` / `btnDanger` (+ Sm variants) — 3-tier moderation button hierarchy
- [x] `fd-avatar` — 32–34px round avatar chip (gradient/color fill, gold border, initials or avatarUrl)
- [x] `arow` — table/list row with gold hover wash + gold-tinted top border
- [x] `fd-switch` / knob — real sliding toggle for feature flags
- [x] `badge-rect` + rank-chip — rounded-rect badge variant + per-tier colored rank chip
- [x] `card-header` — inner card header bar (title left, dim subtitle right, divider)
- [x] `thead-raised` — thead #1e1338 wash
- [x] `flag-banner` — pulsing danger alert bar (Matches)
- [x] `quote-flagged` — red-tinted excerpt/quote box (Moderation)
- [x] Verify: admin typecheck + build stay green after CSS additions

## Phase 1 — Matches (54, the only structural failure)
- [x] Rebuild Matches.tsx to the approved secMatches anti-cheat review-queue shell (flag-banner, queue columns, per-row + drawer decision buttons rendered — disabled/"Phase 2" where the detection backend doesn't exist yet, honestly)
- [x] Verify typecheck + build

## Phase 2 — Moderate layout pages
- [x] Settings (62): fd-2col flags+constants row + real fd-switch toggles
- [x] Economy (72): fd-2col grant+ledger row
- [x] Admins (72): fd-avatar + KPI colors + invite/grant accent
- [x] Support/Tickets (88 but moderate): wire Reopen action + priority display (needs a small server field + reopen path — scope check first)
- [x] Verify each: typecheck + build

## Phase 3 — Trivial token-swap polish sweep (now that tokens exist)
- [x] Overview + Analytics: colored KPI trend deltas (real prior-window counts, no pipeline)
- [x] Players: fd-avatar + Joined column + rank chip + centered status
- [x] Moderation: button hierarchy + quote-flagged box + badge-rect
- [x] Store: row thumbnails
- [x] Guilds: row action column
- [x] Tournaments: per-tile KPI colors + Cinzel form title
- [x] Audit: card-header + append-only count + thead-raised
- [x] LiveOps: remaining token swaps
- [x] Verify: full admin typecheck + build

## Ship
- [x] Whole-branch review of the re-skin diff
- [x] Merge + deploy + verify pages render on app.filipinodama.com
